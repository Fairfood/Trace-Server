...
class DigitalReceiptPolicy(GuardianPolicyBase):
    """Concrete class for Guardian Digital Receipt policy."""

    def build_submission_data(self, transaction: ExternalTransaction) -> dict:
        """Build data specific to Digital Receipt policy."""
        evidence_type, evidence_hash = self.get_evidence_file_hash(transaction)
        data = {
            "document": {
    ...
    def send_trans_data(self, transaction: ExternalTransaction) -> dict:
        ...
        response = requests.post(
            url=policy_const.DIG_REC_SEND_TRANS,
    ...
    def get_claim_status(self, filter_value) -> int:
        ...
        data = self.get_data(
            policy_const.DIG_REC_LIST_TRANS,
            policy_const.DIG_REC_FILTER_TRANS,
            filter_value
        )
        if not data:
    ...
    def update_token(self, guardian_claim_id: str, document_id: str):
        ...
        data = self.get_data(
            policy_const.DIG_REC_LIST_TOKEN, 
            policy_const.DIG_REC_FILTER_TOKEN,
            document_id
        )
        if not data:
            return
        ...
        token = self.get_token_details(
            policy_const.DIG_REC_TRUST_CHAIN, 
            hash_value
        )
        if not token:
            return
    ...
    def set_role(self, role: str):
        ...
        response = requests.post(
            url=policy_const.DIG_REC_SET_ROLE,
            headers=self._get_headers(),
            data=data
        )
        return
    ...
    def continue_policy_workflow(
        ...
        verified = sender.verify_data(
            policy_const.DIG_REC_VERIFY_TRANS, 
            policy_const.DIG_REC_LIST_TRANS,
            policy_const.DIG_REC_FILTER_TRANS,
