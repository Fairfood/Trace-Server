...
class LivingIncomePolicy(GuardianPolicyBase):
    """Concrete class for Living Income policy."""

    def build_submission_data(self, node: Node) -> dict:
        """Build data specific to Living Income policy."""
        data = {
            "document": {
    ...
    def send_data(self, node: Node) -> dict:
        ...
        response = requests.post(
            url=policy_const.LIV_INC_SEND_DATA,
    ...
    def get_claim_status(self, filter_value) -> int:
        ...
        data = self.get_data(
            policy_const.LIV_INC_LIST_DATA,
            policy_const.LIV_INC_FILTER_DATA,
            filter_value
        )
        if not data:
    ...
    def update_token(self, guardian_claim_id: str, document_id: str):
        ...
        data = self.get_data(
            policy_const.LIV_INC_LIST_TOKEN, 
            policy_const.LIV_INC_FILTER_TOKEN,
            document_id
        )
        if not data:
            return
        ...
        token = self.get_token_details(
            policy_const.LIV_INC_TRUST_CHAIN, 
            hash_value
        )
        if not token:
            return
